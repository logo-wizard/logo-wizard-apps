...
async def download_from_s3(base_url: str, path: str, filename: str, expected_size_bytes: float) -> None:
    """
    Downloads a file from an object storage asynchronously and stores it locally into `filename`
    `expected_size_bytes` is used only to show the progress of the download
    """
    ...
    async with ClientSession(base_url, timeout=ClientTimeout(60)) as session:
        async with session.get(path) as resp:
            ...
            async with aiofiles.open(filename, 'wb') as fd:
                while True:
                    chunk = await resp.content.read(1024 ** 2 * 10)
...
async def download_colorization_model_weights_from_s3(
    ...
    """ Downloads colorization model weights from the object storage """
