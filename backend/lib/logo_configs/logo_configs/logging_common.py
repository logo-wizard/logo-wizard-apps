...
def format_frame_locals(
        frame, max_length=512, infix='...',
        prefix_tpl='      %s: ', suffix_tpl=' // %d bytes'):
    ret = ['    Locals:\n']
...
def format_exception_with_locals(exc_type, exc_value, tb):
    try:
        ...
        formatted = (
            ["Couldn't print detailed traceback.\n"] +
            traceback.format_exception(exc_type, exc_value, tb))
        return formatted


def get_record_exc_repr(
        record, fmt=FILE_FORMAT, datefmt=DT_FORMAT, full=False,
        show_locals=False):
    """
